...
__global__ void CalculateFlux(float* d_body, float* dens, float* xv, float* yv, float* zv, float* press,
	...
	if (i < N) {
		...
		if (d_body[(i + 1)] == 0.0) { // right is body
			...
			FR[i + 0 * N] = 0.5*(E[i + 0 * N] + E[i + 1 + 0 * N]) - speed*(U[i + 0 * N] - U[i + 1 + 0 * N]);
			FR[i + 1 * N] = 0.5*(E[i + 1 * N] + E[i + 1 + 1 * N]) - speed*(U[i + 1 * N] - U[i + 1 + 1 * N]);
			FR[i + 2 * N] = 0.5*(E[i + 2 * N] + E[i + 1 + 2 * N]) - speed*(U[i + 2 * N] - U[i + 1 + 2 * N]);
			FR[i + 3 * N] = 0.5*(E[i + 3 * N] + E[i + 1 + 3 * N]) - speed*(U[i + 3 * N] - U[i + 1 + 3 * N]);
			FR[i + 4 * N] = 0.5*(E[i + 4 * N] + E[i + 1 + 4 * N]) - speed*(U[i + 4 * N] - U[i + 1 + 4 * N]);
		}
		...
		if (d_body[i + NX] == 0.0) { // front is body
			...
			FF[i + 0 * N] = 0.5*(F[i + 0 * N] + F[i + NX + 0 * N]) - speed*(U[i + 0 * N] - U[i + NX + 0 * N]);
			FF[i + 1 * N] = 0.5*(F[i + 1 * N] + F[i + NX + 1 * N]) - speed*(U[i + 1 * N] - U[i + NX + 1 * N]);
			FF[i + 2 * N] = 0.5*(F[i + 2 * N] + F[i + NX + 2 * N]) - speed*(U[i + 2 * N] - U[i + NX + 2 * N]);
			FF[i + 3 * N] = 0.5*(F[i + 3 * N] + F[i + NX + 3 * N]) - speed*(U[i + 3 * N] - U[i + NX + 3 * N]);
			FF[i + 4 * N] = 0.5*(F[i + 4 * N] + F[i + NX + 4 * N]) - speed*(U[i + 4 * N] - U[i + NX + 4 * N]);
		}
	...
	if (i < N) {
		...
		//Renew left and right boundary condition
			...
			U_new[i + (NX - 1) + 0 * N] = U_new[i + (NX - 2) + 0 * N];
			U_new[i + (NX - 1) + 1 * N] = U_new[i + (NX - 2) + 1 * N];
			U_new[i + (NX - 1) + 2 * N] = U_new[i + (NX - 2) + 2 * N];
			U_new[i + (NX - 1) + 3 * N] = U_new[i + (NX - 2) + 3 * N];
			U_new[i + (NX - 1) + 4 * N] = U_new[i + (NX - 2) + 4 * N];
		}
	}
	__syncthreads();
