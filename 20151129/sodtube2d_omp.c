   ...
   for(i = 0;i < NX;i++)
   {
	for( j = 0;j < NY;j++)
	{
	  ...
	  U[i + j*NX][2] = dens[i + j*NX] * (CV*temp[i + j*NX]
 	  ...
 	  U[i+j*NX][3] = dens[i+j*NX]*yv[i+j*NX];
	}
   }
}
void CalculateFlux()
...
FD[i+j*NX][0] = 0.5*(dens[i+(j-1)*NX]*yv[i+(j-1)*NX] + dens[i+j*NX]*yv[i+j*NX] ) - 
				speed*(U[i+j*NX][0] - U[i+(j-1)*NX][0]);
FU[i+j*NX][0] = 0.5*(dens[i+j*NX]*yv[i+j*NX] + dens[i+(j+1)*NX]*yv[i+(j+1)*NX] ) - 
